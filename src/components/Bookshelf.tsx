...
export function Bookshelf({ onOpenProject, onContinueProject, onNewProject }: BookshelfProps) {
  ...
  const [iconMenuId, setIconMenuId] = useState<string | null>(null);
  ...
  const handleRenameStart = (e: React.MouseEvent, p: NovelProject) => {
    e.stopPropagation();
  ...
  const handleRenameSave = (id: string) => {
    const updated = projects.map(p =>
      p.id === id ? { ...p, title: editTitleValue.trim() || '未命名', updatedAt: Date.now() } : p
    );
    localStorage.setItem('novel_history', JSON.stringify(updated));
    setProjects(updated);
  ...
  const handleIconChange = (e: React.MouseEvent, id: string, icon: CoverIcon) => {
    e.stopPropagation();
    const updated = projects.map(p =>
      p.id === id ? { ...p, coverIcon: icon } : p
    );
    localStorage.setItem('novel_history', JSON.stringify(updated));
    setProjects(updated);
    setIconMenuId(null);
  ...
  const renderCoverIcon = (p: NovelProject) => {
    const icon = p.coverIcon || 'none';
    if (icon === 'heart') return <Heart className="h-5 w-5 fill-red-400 text-red-400 drop-shadow" />;
    if (icon === 'star') return <Star className="h-5 w-5 fill-yellow-400 text-yellow-400 drop-shadow" />;
    return null;
  };
                ...
                <div key={p.id} className="group relative">
                  ...
                  >
                    ...
                    <div className="absolute top-2 right-2">
                      <div
                        onClick={(e) => { e.stopPropagation(); setIconMenuId(iconMenuId === p.id ? null : p.id); }}
                        className="cursor-pointer hover:scale-125 transition-transform"
                      >
                        {renderCoverIcon(p) || (
                          <div className="h-5 w-5 rounded-full border border-white/20 flex items-center justify-center opacity-0 group-hover:opacity-50 transition-opacity">
                            <Plus className="h-3 w-3 text-white/50" />
                          </div>
                        )}
                      </div>
                      {iconMenuId === p.id && (
                        <div className="absolute top-7 right-0 bg-card border border-border rounded-md shadow-lg p-1 flex gap-1 z-10">
                          {coverIconOptions.map(opt => (
                            <button
                              key={opt.value}
                              onClick={(e) => handleIconChange(e, p.id, opt.value)}
                              className={`p-1.5 rounded hover:bg-accent transition-colors text-xs ${p.coverIcon === opt.value ? 'bg-accent' : ''}`}
                              title={opt.value === 'none' ? '无图标' : opt.label}
                            >
                              {opt.icon || <X className="h-4 w-4 text-muted-foreground" />}
                            </button>
                          ))}
                        </div>
                      )}
                  ...
                  <div className="absolute -bottom-1 left-0 right-0 flex justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <Button variant="secondary" size="icon" className="h-7 w-7 shadow-md"
                      onClick={(e) => handleRenameStart(e, p)} title="改名">
                  ...
                  {editingTitleId === p.id ? (
                    <div className="mt-2 flex gap-1" onClick={(e) => e.stopPropagation()}>
                      <Input
                        ...
                        onKeyDown={(e) => { if (e.key === 'Enter') handleRenameSave(p.id); if (e.key === 'Escape') setEditingTitleId(null); }}
                        className="h-7 text-xs flex-1"
                        autoFocus
                      />
                      <Button size="icon" className="h-7 w-7" onClick={() => handleRenameSave(p.id)}>
